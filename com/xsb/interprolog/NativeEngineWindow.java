...
/** A ListenerWindowX for a NativeEngine */
public class NativeEngineWindow extends ListenerWindow {
	
	public NativeEngineWindow(NativeEngine e) throws IOException{
		this(e,true);
		setTitle("NativeEngine listener ("+e.getPrologVersion()+")");
	}
	public NativeEngineWindow(NativeEngine e,boolean autoDisplay) throws IOException{
		super(e,autoDisplay);
    ...
    @Override
    public void sendCallToProlog(String s) {
                    		String goal = prologInput.getText().trim();
		if (goal.equals(";")) {
			beep();
			prologOutput.append("Sorry, no multiple solutions available with NativeEngine; use SubprocessEngine if you need them\n");
